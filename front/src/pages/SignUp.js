...
const SignUp = () => {
    ...
    const handleSignup=async(e)=>{
        e.preventDefault();
        if(!image) return alert('사진 올려주세요')
        const url= await uploadImage(image)
        console.log(url)
        singupUser({name,email,password,picture: url}).then(({data})=>{
          if(data){
            console.log(data)
            navigate('/chat')
          }
    ...
    const [singupUser,{isLoading,error}]= useSignupUserMutation()
        ...
        <Col md={7} className='d-flex align-items-center justify-content-center flex-direction-column'>
          <Form style={{width:'80%', maxWidth:500}} onSubmit={handleSignup}>
            ...
            <div className='signup-profile-pic__container'>
                <img src={ imagePreview || bot} className='signup-profile-pic'></img>
